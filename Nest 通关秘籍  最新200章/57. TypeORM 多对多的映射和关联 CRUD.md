...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-1.png)

Entity 之间的引用关系，转换为数据库表之间的外键关联的关系。

一对多我们是通过 @OneToMany 和 @ManyToOne 来把 Entity 映射成数据库表：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-2.png)

它并不需要 @JoinColumn 来指定外键列，因为外键一定在多的那一边。

那多对多呢？

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-3.png)

前面讲过，在数据库里，我们是通过中间表来保存这种多对多的关系的：

把多对多拆成了两个一对多：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-4.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-5.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-6.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-7.png)

把 index.ts 的代码去掉：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-8.png)

然后 npm run start

可以看到它生成了两个 Entity 的建表 sql：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-9.png)

然后把这两个表删掉，我们来添加多对多的关联关系：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-10.png)

在 Entity 里通过 @ManyToMany 关联。

比如一篇文章可以有多个标签：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-11.png)

然后再 npm run start

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-12.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-13.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-14.png)

可以看到，3 个标签、2 篇文章，还有两者的关系，都插入成功了。

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-15.png)

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-16.png)

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-17.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-18.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-19.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-20.png)

npm run start 跑一下：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-21.png)

它会先查出 id 为 2 的 article 有哪些标签，查出了 1、2、3。

然后会把他和 id 为 2 的 article 的关系，(2, 2) (2, 3) 从中间表中删除。

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-22.png)

这样就这个 article 就只有 id 为 1 的 tag 了。

此外，更新 article.title 的是另一个 update 语句：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-23.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-24.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-25.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-26.png)

一对多的 department 那方，不维护外键，所以需要第二个参数来指定通过哪个外键找到 department：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-27.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/57-28.png)
