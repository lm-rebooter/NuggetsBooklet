...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-1.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-2.png)

随便找个有 websocket 的网站，比如知乎就有：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-3.png)

过滤出 ws 类型的请求，看看这几个 header，是不是就是前面说的那些。

这个 Sec-WebSocket-Key 是 wk60yiym2FEwCAMVZE3FgQ==

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-4.png)

而响应的 Sec-WebSocket-Accept 是 XRfPnS+8xl11QWZherej/dkHPHM=

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-5.png)

我们算算看：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-6.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-7.png)

全新的协议？那具体是什么样的协议呢？

这样的：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-8.png)

大家习惯的 http 协议是 key:value 的 header 带个 body 的：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-9.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-10.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-11.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-12.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-13.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-14.png)

这 3 个 header 还有 101 状态码都是我们返回的。

message 里也可以看到发送的消息：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-15.png)

再去服务端看看，也收到了这个消息：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-16.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-17.png)

我们需要第一个字节的后四位，也就是 opcode。

这样写：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-18.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-19.png)

第一位是 mask 标志位，后 7 位是 payload 长度。

可以这样取：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-20.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-21.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-22.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-23.png)

这样就拿到了 payload 的长度，然后再用这个长度去截取内容就好了。

但在读取数据之前，还有个 mask 要处理，这个是用来给内容解密的：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-24.png)

读 4 个字节，就是 mask key。

再后面的就可以根据 payload 长度读出来。

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-25.png)
```javascript
let realData = null;
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-26.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-27.png)

而现在我们能正确解析出其中的数据：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-28.png)

至此，我们 websocket 协议的解析成功了！

这样的协议格式的数据叫做 frame，也就是帧：

![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-29.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-30.png)
```javascript
send(data) {
    let opcode;
    let buffer;
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-31.png)
```javascript
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-32.png)
...
![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-33.png)

收到客户端消息后，每两秒回一个消息。


![](//liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/172-34.png)
